...
from dataloader import IEMOCAPDataset, MELDDataset, M3EDDataset
from sklearn import metrics
from sklearn.metrics import f1_score, accuracy_score
from torch.utils.data import DataLoader
from torch.utils.data.sampler import SubsetRandomSampler
from PCGNet import GATModel
from loss import FocalLoss, GradNormLoss, MinNormSolver, PCGrad_backward, Pareto, MM
...
def find_hard_negatives(emotion_features, emotion_labels, lengths):
    hard_negatives = []

    start_idx = 0
    for dialog_length in lengths:
        end_idx = start_idx + dialog_length
        dialog_features = emotion_features[start_idx:end_idx]
        dialog_labels = emotion_labels[start_idx:end_idx]

        for i in range(dialog_length):
            target_label = dialog_labels[i]
            negative_indices = [j for j in range(dialog_length) if dialog_labels[j] != target_label]

            hard_negatives.append({
                'sentence_index': start_idx + i,
                'hard_negative_indices': [start_idx + idx for idx in negative_indices],
                'hard_negative_features': dialog_features[negative_indices],
                'hard_negative_labels': dialog_labels[negative_indices]
            })
...
def get_hard_negative_weights(emotion_features, hard_negatives, weight=1):
    ...
    for item in hard_negatives:
        ...
        for neg_idx in item['hard_negative_indices']:
            hard_negative_weights[sentence_idx, neg_idx] = weight  # 设置为较高的权重
    return hard_negative_weights.to(emotion_features.device)
...
def get_M3ED_loaders(data_path=None, batch_size=32, valid_rate=0.1, num_workers=0, pin_memory=False, use_multiemo=False):
    trainset = M3EDDataset(path=data_path, use_multiemo=use_multiemo)
    train_sampler, valid_sampler = get_train_valid_sampler(trainset, valid_rate)
    ...
    testset = M3EDDataset(path=data_path, train=False, use_multiemo=use_multiemo)
    test_loader = DataLoader(testset,
                             batch_size=batch_size,
                             collate_fn=testset.collate_fn,
                             num_workers=num_workers,
...
def getLoss(loss_erc, person_loss_shift, context_loss_shift, loss_shift_cl, loss_emotion_cl1,loss_emotion_cl2, device, train_flag, optimizer, features_shift, features_erc, scheduler, wo_pareto):
    ...
    if train_flag == True and wo_pareto == False:
        ...
        total_loss = 0
        for l in losses:
            weight = 1 / float(l) if float(l) != 0 else 0
            total_loss += l * weight
        total_loss.backward()
        optimizer.step()
    else:
...
def build_anchor_contrast_mask(features, modal_dim=200):
    """
    输入: features [bsz, modal_dim * 3]
    输出: anchor [bsz, dim], contrast [2*bsz, dim], mask [bsz, 2*bsz]
    """
    bsz = features.shape[0]

    text_feat = features[:, :modal_dim]              # [bsz, 200]
    audio_feat = features[:, modal_dim:2*modal_dim]  # [bsz, 200]
    video_feat = features[:, 2*modal_dim:]           # [bsz, 200]
    ...
    return anchor_feat, contrast_feat, mask


def train_or_eval_graph_model(model, loss_f, dataloader, epoch=0, wo_pareto=False, wo_ercByShiftEdges=False, train_flag=False, optimizer=None, scheduler=None, cuda_flag=False, modals=None, target_names=None,
                              test_label=False, tensorboard=False):
    losses, preds, labels, hiddens = [], [], [], []
    person_shift_labels, context_shift_labels, person_shift_preds, context_shift_preds = [], [], [], []
    scores, vids = [], []
    ...
    for data in dataloader:
        ...
        if args.dataset == 'M3ED':
            log_prob, person_log_prob_shift, context_log_prob_shift, hidden, person_hidden_shift, context_hidden_shift, features_shift, features_erc = model(
                [textf1, textf2, textf3, textf4], qmask, umask,
                lengths, acouf, visuf, speaker=speaker,
                wo_ercByShiftEdges=wo_ercByShiftEdges)
        else:
            log_prob, person_log_prob_shift, context_log_prob_shift, hidden, person_hidden_shift, context_hidden_shift, features_shift, features_erc = model(
            [textf1, textf2, textf3, textf4], qmask, umask,
            lengths, acouf, visuf, speaker=speaker,
            persona=persona, wo_ercByShiftEdges=wo_ercByShiftEdges)

        device = person_log_prob_shift.device

        loss_erc = loss_f['erc'](input=log_prob, target=label)
        person_loss_shift = loss_f['shift'](person_log_prob_shift, person_label_shift)
        context_loss_shift = loss_f['shift'](context_log_prob_shift, context_label_shift)


        if args.wo_shiftcl and args.wo_emotioncl:
            loss = getLoss(loss_erc, person_loss_shift, context_loss_shift, torch.zeros(1).to(device), torch.zeros(1).to(device), device, train_flag, optimizer, features_shift, features_erc, scheduler, wo_pareto)
           
        elif args.wo_shiftcl == True and args.wo_emotioncl == False:
            hard_negatives = find_hard_negatives(emotion_features=hidden, emotion_labels=label, lengths=lengths)
            hard_negative_weights = get_hard_negative_weights(emotion_features=hidden, hard_negatives=hard_negatives, weight=args.weight)
            loss_emotion_cl = loss_f['emotion_cl'](features=hidden, labels=label,weight=create_class_weight_SCL(label),
                                                 hard_negative_weights=hard_negative_weights)

            loss = getLoss(loss_erc, person_loss_shift, context_loss_shift, torch.zeros(1).to(device), loss_emotion_cl, device, train_flag, optimizer, features_shift, features_erc, scheduler, wo_pareto)
            
        elif args.wo_shiftcl == False and args.wo_emotioncl == True:
            shift_features_pair, shift_features_pair_label = shiftContrastLearningShiftTask(
                shift_features=context_hidden_shift, shift_labels=context_label_shift, sample_num=args.sample_num,
                dataset=args.dataset)
            loss_shift_cl = loss_f['shift_cl'](features=shift_features_pair, labels=shift_features_pair_label,
                                               weight=create_class_weight_SCL(shift_features_pair_label))

            loss = getLoss(loss_erc, person_loss_shift, context_loss_shift, loss_shift_cl, torch.zeros(1).to(device), device, train_flag, optimizer, features_shift, features_erc, scheduler, wo_pareto)
            ...
            hard_negatives = find_hard_negatives(emotion_features=hidden, emotion_labels=label, lengths=lengths)
            hard_negative_weights = get_hard_negative_weights(emotion_features=hidden,
                                                              hard_negatives=hard_negatives,
                                                              weight=args.weight)
            loss_emotion_cl1 = loss_f['emotion_cl'](features=hidden, labels=label,
                                                 hard_negative_weights=hard_negative_weights)


            modal_count = 3
            modal_dim = hidden.shape[1] // modal_count


            # features = hidden.view(-1, modal_count, modal_dim).view(-1, modal_dim)  # [975, 200]

            # ids = torch.arange(hidden.shape[0]).repeat_interleave(modal_count)
            # mask = torch.eq(ids.unsqueeze(0), ids.unsqueeze(1)).float()

            # 构建 anchor, contrast, mask
            anchor, contrast, mask = build_anchor_contrast_mask(hidden, modal_dim=modal_dim)
            loss_emotion_cl2 = loss_f['emotion_cl_2'](features=hidden, mask=mask, anchor=anchor, contrast=contrast)


            # loss_emotion_cl2 = loss_f['emotion_cl_2'](features=features, mask=mask)


            # loss_emotion_cl = loss_emotion_cl2
            loss = getLoss(loss_erc, person_loss_shift, context_loss_shift, loss_shift_cl, loss_emotion_cl1, loss_emotion_cl2,device, train_flag, optimizer, features_shift, features_erc, scheduler, wo_pareto)
            losses.append( float(loss_erc) + float(person_loss_shift) + float(context_loss_shift) + float(loss_shift_cl) + float(loss_emotion_cl1)+ float(loss_emotion_cl1))
        if log_prob != None:
        ...
        person_shift_labels.append(person_label_shift.cpu().numpy())
        context_shift_labels.append(context_label_shift.cpu().numpy())

        hiddens.append(hidden.detach().cpu().numpy())
    ...
    if preds != []:
        ...
        person_shift_preds = np.concatenate(person_shift_preds)
        context_shift_preds = np.concatenate(context_shift_preds)
        person_shift_labels = np.concatenate(person_shift_labels)
        context_shift_labels = np.concatenate(context_shift_labels)
        if not train_flag :
            print("person shift metrics: ACC = {}, F1 score = {}".format(accuracy_score(person_shift_preds, person_shift_preds), f1_score(person_shift_labels, person_shift_preds, average='weighted')))
            print("context shift metrics: ACC = {}, F1 score = {}".format(accuracy_score(context_shift_labels, context_shift_preds), f1_score(context_shift_labels, context_shift_preds, average='weighted')))
    else:
    ...
    return all_each, all_acc, avg_loss, avg_accuracy, labels, preds, avg_fscore, [vids, ei, et, en, el], hiddens, person_shift_preds, context_shift_preds, person_shift_labels, context_shift_labels
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wo_persona', default=True, action='store_true')
    parser.add_argument('--wo_crosstask', default=True, action='store_true')
    parser.add_argument('--wo_shiftcl', default=False, action='store_true')
    parser.add_argument('--wo_emotioncl', default=False, action='store_true')
    parser.add_argument('--wo_ercByShiftEdges', default=False,action='store_true')
    parser.add_argument('--wo_pareto', default=False, action='store_true')
    ...
    modals = args.modals
    feat2dim = {'IS10': 1582, '3DCNN': 512, 'textCNN': 100, 'bert': 768, 'denseface': 342, 'MELD_text': 600, 'MELD_audio': 300, 'Wav2Vec': 1024}
    if args.dataset == 'M3ED':
        D_audio = 1024
    ...
    if args.dataset == 'MELD':
        ...
        class_weights = torch.FloatTensor([1.0 / 0.466750766,
                                           ...
                                           1.0 / 0.113714183])
    if args.dataset == 'M3ED':
        n_speakers, n_classes = 2, 7

        target_names = ['hap', 'neu', 'sad', 'dis', 'ang', 'fea', 'sur']
        class_weights = torch.FloatTensor([1.0 / 0.0936,
                                           1.0 / 0.4102,
                                           1.0 / 0.1619,
                                           1.0 / 0.0612,
                                           1.0 / 0.2141,
    ...
    model = GATModel(base_model=args.base_model, D_m=D_m, D_m_v=D_visual, D_m_a=D_audio, n_speakers=n_speakers,
                ...
                av_using_lstm=args.av_using_lstm, norm='', wo_persona=args.wo_persona, wo_crosstask=args.wo_crosstask, wo_shiftcl=args.wo_shiftcl, wo_emotioncl=args.wo_emotioncl, wo_pareto=args.wo_pareto,  wo_ercByShiftEdges=args.wo_ercByShiftEdges, weight=args.weight
        )
    ...
    loss_f['emotion_cl'] = CKSCL()
    loss_f['emotion_cl_2'] = CKSCL()
    ...
    if args.dataset == 'MELD':
        ...
        train_loader, valid_loader, test_loader = get_M3ED_loaders(data_path=args.data_dir,
                                                                      valid_rate=args.valid_rate,
                                                                      batch_size=batch_size,
                                                                      num_workers=0,
                                                                      use_multiemo=args.use_multiemo)
    ...
    if args.test_label and args.graph_model:
        ...
        all_each, all_acc, test_loss, test_acc, test_label, test_pred, test_fscore, _, hidden, person_shift_preds, context_shift_preds, person_shift_labels, context_shift_labels = train_or_eval_graph_model(model=model,
                                                                                                                  loss_f=loss_f,
    ...
    all_test_fscore, all_test_acc, all_all_each, all_erc_confusion_matrices, all_erc_test_feature, all_erc_test_label, all_erc_test_pred = [], [], [], [], [], [], []
    all_es_person_test_feature, all_es_person_test_label, all_es_context_test_feature, all_es_context_test_label, all_es_person_confusion_matrices, all_es_context_confusion_matrices = [], [], [], [], [], []
    ...
    for e in range(n_epochs):
        ...
        _, _, train_loss, train_acc, _, _, train_fscore, _, _, _, _, _, _ = train_or_eval_graph_model(model=model,
                                                                                        loss_f=loss_f,
                                                                                        dataloader=train_loader,
                                                                                        epoch=e,
                                                                                        wo_pareto=args.wo_pareto,
                                                                                        wo_ercByShiftEdges=args.wo_ercByShiftEdges,
                                                                                        train_flag=True,
                                                                                        optimizer=optimizer,
                                                                                        scheduler=scheduler,
                                                                                        cuda_flag=cuda_flag,
                                                                                        modals=args.modals,
                                                                                        target_names=target_names)
        _, _, valid_loss, valid_acc, _, _, valid_fscore, _, _, _, _, _, _ = train_or_eval_graph_model(model=model,
                                                                                        loss_f=loss_f,
                                                                                        ...
                                                                                        modals=args.modals,
                                                                                        target_names=target_names)
        all_each, all_acc, test_loss, test_acc, test_label, test_pred, test_fscore, _, hiddens, person_shift_preds, context_shift_preds, person_shift_labels, context_shift_labels = train_or_eval_graph_model(model=model,
                                                                                                                    loss_f=loss_f,
                                                                                                                    dataloader=test_loader,
                                                                                                                    epoch=e,
                                                                                                                    wo_pareto=args.wo_pareto,
        ...
        cm_es_person = metrics.confusion_matrix(person_shift_labels, person_shift_preds)
        all_es_person_confusion_matrices.append(cm_es_person)

        cm_es_context = metrics.confusion_matrix(context_shift_labels, context_shift_preds)
    ...
    print('All-each:\n', all_all_each[best_epoch] if best_epoch >= 0 else 0)
