...
def decodeShiftLabel(shift_label, dataset):
    ...
    # front_label = [int(label // class_num) for label in shift_label]
    # back_label = [int(label % class_num) for label in shift_label]
...
def train_or_eval_graph_model(model, loss_f, dataloader, epoch=0, stage=1, wo_ercByShiftEdges=False, train_flag=False, optimizer=None, scheduler=None, cuda_flag=False, modals=None, target_names=None,
    ...
    for data in dataloader:
        ...
        log_prob, person_log_prob_shift, context_log_prob_shift, hidden, person_hidden_shift, context_hidden_shift = model(
        ...
        if stage == 1:
            if args.wo_shiftcl:
                loss = (loss_erc * (1 / float(loss_erc) if float(loss_erc) != 0 else 0) + \
                        person_loss_shift * (1 / float(person_loss_shift) if float(person_loss_shift) != 0 else 0) + \
                        context_loss_shift * (1 / float(context_loss_shift) if float(context_loss_shift) != 0 else 0))
            else:
            ...
            if args.wo_shiftcl and args.wo_emotioncl:
                ...
                loss_emotion_cl = loss_f['shift_cl'](features=hidden, labels=label,
                                                     hard_negative_weights=hard_negative_weights)
                loss = loss_erc * (1 / float(loss_erc) if float(loss_erc) != 0 else 0) + \
                       person_loss_shift * (1 / float(person_loss_shift) if float(person_loss_shift) != 0 else 0) + \
                       context_loss_shift * (1 / float(context_loss_shift) if float(context_loss_shift) != 0 else 0) + \
                       loss_emotion_cl * (1 / float(loss_emotion_cl) if float(loss_emotion_cl) != 0 else 0)
        ...
        loss_shift = person_loss_shift * (1/float(person_loss_shift) if float(person_loss_shift) != 0 else 0) + \
                context_loss_shift * (1 / float(context_loss_shift) if float(context_loss_shift) != 0 else 0)
        if train_flag:
            loss.backward()
            if tensorboard:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wo_shiftcl', default=True, action='store_true')
    ...
    parser.add_argument('--epoch_stage', default=3, type=int)
    ...
    model = GATModel(base_model=args.base_model, D_m=D_m, D_m_v=D_visual, D_m_a=D_audio, n_speakers=n_speakers,
                ...
                av_using_lstm=args.av_using_lstm, norm='', wo_persona=args.wo_persona, wo_crosstask=args.wo_crosstask, wo_shiftcl=args.wo_shiftcl, wo_emotioncl=args.wo_emotioncl, epoch_stage=args.epoch_stage,  wo_ercByShiftEdges=args.wo_ercByShiftEdges, weight=args.weight
        )
    ...
    for e in range(n_epochs):
        ...
        if stage == 2:
            ...
            if patience >= args.patience and patience2 >= args.patience:
                ...
                break
    i = best_epoch - n_epochs // args.epoch_stage
    print('Final Test performance...')
    print('Early stoping...', patience, patience2)
    print('Eval-metric: F1, Epoch: {}, best_eval_fscore: {}, Accuracy: {}, F1-Score: {}'.format(best_epoch, best_eval_fscore,
                                                                                                all_test_acc[i] if best_epoch >= 0 else 0,
                                                                                                all_test_fscore[i] if best_epoch >= 0 else 0))
    print('All-each:\n', all_all_each[i] if best_epoch >= 0 else 0)
